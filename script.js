...
// Show main content only after user explores (keep existing UI unchanged)
function enterMainPage() {
  if (!landing || !mainContent) return;

  // Prevent double clicks
  if (landing.classList.contains("is-exiting")) return;
  exploreBtn?.setAttribute("disabled", "true");

  // Start immersive transition
  landing.classList.add("is-exiting");

  // After animation, swap visibility
  const durationMs = 1400; // grander transition length
  window.setTimeout(() => {
    ...
    // Ensure user lands at top of event page
    try {
...
exploreBtn?.addEventListener("click", enterMainPage);
